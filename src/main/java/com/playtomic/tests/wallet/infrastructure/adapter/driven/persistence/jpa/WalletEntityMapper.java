...
public class WalletEntityMapper {
  ...
  public WalletEntity toEntity(Wallet wallet) {
    return new WalletEntity(
        ...
        wallet.version(),
        wallet.transactions().stream().map(this::transactionToEntity).toList());
  }
  ...
  public Wallet toDomain(WalletEntity wallet) {
    return new Wallet(
        ...
        wallet.getTransactions().stream().map(this::transactionToDomain).toList());
  }

  private TransactionEntity transactionToEntity(Transaction transaction) {
  ...
  private Transaction transactionToDomain(TransactionEntity transaction) {
